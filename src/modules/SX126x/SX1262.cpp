...
int16_t SX1262::begin(float freq, float bw, uint8_t sf, uint8_t cr, uint8_t syncWord, int8_t power, uint16_t preambleLength, float tcxoVoltage, bool useRegulatorLDO) {
  ...
  return(begin({freq, bw, sf, cr, syncWord, power, preambleLength, tcxoVoltage, useRegulatorLDO}));
#else
  // C++11 does not support designated initializers, so we need to do this the old way
...
#if __cplusplus >= 201402L
  return(beginFSK({freq, br, freqDev, rxBw, power, preambleLength, tcxoVoltage, useRegulatorLDO}));
#else
  // C++11 does not support designated initializers, so we need to do this the old way
...
#if __cplusplus >= 201402L
  return(beginLRFHSS({freq, bw, cr, narrowGrid, power, tcxoVoltage, useRegulatorLDO}));
#else
  // C++11 does not support designated initializers, so we need to do this the old way
