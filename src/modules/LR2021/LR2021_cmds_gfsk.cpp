...
int16_t LR2021::setFskModulationParams(uint32_t bitRate, uint8_t pulseShape, uint8_t rxBw, uint32_t freqDev) {
  uint8_t buff[] = { 
  ...
  return(this->SPIcommand(RADIOLIB_LR2021_CMD_SET_FSK_MODULATION_PARAMS, true, buff, sizeof(buff)));
}

int16_t LR2021::setFskPacketParams(uint16_t preambleLen, uint8_t preambleDetect, bool longPreamble, bool pldLenBits, uint8_t addrComp, uint8_t packetFormat, uint16_t payloadLen, uint8_t crc, uint8_t dcFree) {
  uint8_t buff[] = { 
  ...
  return(this->SPIcommand(RADIOLIB_LR2021_CMD_SET_FSK_PACKET_PARAMS, true, buff, sizeof(buff)));
}

int16_t LR2021::setFskWhiteningParams(uint8_t whitenType, uint16_t init) {
  uint8_t buff[] = { 
    (uint8_t)((whitenType << 4) | (uint8_t)((init >> 8) & 0x0F)),
    (uint8_t)(init & 0xFF),
  };
  return(this->SPIcommand(RADIOLIB_LR2021_CMD_SET_FSK_WHITENING_PARAMS, true, buff, sizeof(buff)));
}

int16_t LR2021::setFskCrcParams(uint32_t poly, uint32_t init) {
  uint8_t buff[] = { 
  ...
  return(this->SPIcommand(RADIOLIB_LR2021_CMD_SET_FSK_CRC_PARAMS, true, buff, sizeof(buff)));
}

int16_t LR2021::setFskSyncword(uint8_t* syncWord, size_t syncWordLen, bool msbFirst) {
  uint8_t buff[9] = { 0 };
...
int16_t LR2021::setFskAddress(uint8_t addrNode, uint8_t addrBroadcast) {
  uint8_t buff[] = { addrNode, addrBroadcast };
  return(this->SPIcommand(RADIOLIB_LR2021_CMD_SET_FSK_ADDRESS, true, buff, sizeof(buff)));
}

int16_t LR2021::getFskRxStats(uint16_t* packetRx, uint16_t* packetCrcError, uint16_t* lenError, uint16_t* preambleDet, uint16_t* syncOk, uint16_t* syncFail, uint16_t* timeout) {
  uint8_t buff[14] = { 0 };
...
int16_t LR2021::getFskPacketStatus(uint16_t* packetLen, float* rssiAvg, float* rssiSync, bool* addrMatchNode, bool* addrMatchBroadcast, float* lqi) {
  uint8_t buff[6] = { 0 };
  int16_t state = this->SPIcommand(RADIOLIB_LR2021_CMD_GET_FSK_RX_STATS, false, buff, sizeof(buff));
  uint16_t raw = 0;
