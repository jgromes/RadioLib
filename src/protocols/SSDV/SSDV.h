/*
 * SSDVClient.h — SSDV (Slow Scan Digital Video) transmit client for RadioLib
 *
 * Encodes a JPEG image into the SSDV packet format defined at
 *   https://ukhas.org.uk/doku.php?id=guides:ssdv
 * and transmits each 256-byte packet via any RadioLib PhysicalLayer radio.
 *
 * Depends on:
 *   • fsphil/ssdv  — ssdv.h / ssdv.c  (packet building + JPEG re-encoding)
 *   • rs8.h / rs8.c (Phil Karn KA9Q Reed-Solomon codec, as used by TBTracker)
 *
 ...
 *
 * Author: generated for use with jgromes/RadioLib
 * License: MIT
 */
...
/*!
 * \class SSDVClient
 * \brief SSDV image transmitter for RadioLib.
 * \note LoRa (SX127x / SX126x) has a 255-byte payload limit.  SSDV packets
...
public:
   ...
   *       (SX1278, SX1262, CC1101, …).  Ownership is NOT transferred;
   *       the radio must remain valid for the lifetime of this object.
   ...
   * \returns \ref status_codes — RADIOLIB_ERR_NONE on success.
   */
   ...
   * \returns \ref status_codes — RADIOLIB_ERR_NONE on success.
   *      The packet count is then available via numPackets().
   */
   ...
   *      • RADIOLIB_ERR_SSDV_NOT_INITIALIZED — begin() not called
   *      • RADIOLIB_ERR_SSDV_NO_PACKET_BUFFER — setImage() not called
   *      • RADIOLIB_ERR_SSDV_ALL_SENT — all packets already sent
   *      • RADIOLIB_ERR_SSDV_TRANSMIT_FAILED — radio TX error
   *      (see getLastRadioError() for the underlying code)
   */
   ...
   *
   * \param packetId  Optional; same semantics as transmit().
   * \returns Same semantics as transmit().
   */
  int16_t transmitLoRa(uint16_t* packetId = NULL);

  /*!
   * \brief Reset the transmit pointer back to packet 0.
   *
   * Allows the same encoded image to be transmitted again without
   * re-encoding.  The image buffer is retained.
   */
  void rewind();
