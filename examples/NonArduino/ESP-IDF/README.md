...
This example shows how to use RadioLib as ESP-IDF component, without the Arduino framework. To run in ESP-IDF (or on any other platform), RadioLib includes an internal hardware abstraction layer (HAL). This software layer takes care of basic interaction with the hardware, such as performing SPI transaction or GPIO operations. The ESP-IDF HAL ships with RadioLib in `src/hal/ESP-IDF/EspHal.{h,cpp}` and is built automatically when RadioLib is included as an ESP-IDF component.
