...
void setup() {
  ...
  if (!rtc_gpio_is_valid_gpio((gpio_num_t)SX1262_DIO1)) {
    Serial.println("[ESP32] Deep sleep requires DIO1 be wired to a RTC GPIO");
    while (true) { delay(10); }
  }
  ...
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT1) {
    ...
    resetModule = false; // Don't reset the radio if we woke due to its interrupt
    receivedFlag = true; // Set the interrupt flag immediately
  }
